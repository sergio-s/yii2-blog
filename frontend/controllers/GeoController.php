    ...
    public function actionCities($cityId)
    {
        ...
        if(isset($city->geoInstitutions) && NULL != $city->geoInstitutions){
            ...
            foreach($city->geoInstitutions as $institution){
                ...
                $markerMap[$i]['infowindow']['content']   = "<div>";
                if(isset($institution->geoInstitutionsPhotos[0]->img) && NULL != $institution->geoInstitutionsPhotos[0]->img){
                    $markerMap[$i]['infowindow']['content']  .= "<img style='width:60px; hight:60px;float:left;margin-right:4px;' src='";//$institution->geoInstitutionsPhotos
                    $markerMap[$i]['infowindow']['content']  .= Yii::getAlias("@web/img/geo/institution-{$institution->id}/{$institution->geoInstitutionsPhotos[0]->img}");
                    $markerMap[$i]['infowindow']['content']  .= "'>";
                }
                $markerMap[$i]['infowindow']['content']  .= "<p style='overflow:hidden'>";
                $markerMap[$i]['infowindow']['content']  .= "Подробнее по роддому <br>";
                $markerMap[$i]['infowindow']['content']  .= "<a href='".Url::toRoute(['/geo/institutions', 'instId' => $institution->id])."'>{$institution->name}</a> ";
                $markerMap[$i]['infowindow']['content']  .= "</p>";
                $markerMap[$i]['infowindow']['content']  .= "</div>";
                $i++;
           }
        }
